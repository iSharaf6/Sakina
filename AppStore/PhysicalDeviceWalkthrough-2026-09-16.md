...
This draft now describes **Haneen 1.0 (2)**, including unified email sign-in and external SoundCloud links. Build 2 is not yet uploaded. The saved App Store Connect notes still describe build 1 and need replacing after the new build and evidence are ready. Production email delivery configuration and physical account-flow verification remain pending.
...
| 6. Du'as and dhikr | Open **Du'as → Morning**. Show Arabic/meaning, source/context and reported virtue where that item has one. Tap its count control when present. Briefly tap **Listen on SoundCloud** to show the external original-recording page, then return to Haneen. Evening has the same type of link. Return to Home, scroll to **Tasbih**, choose a dhikr and increment it. | Sourced religious readings, local counters and a clearly external recording link. Haneen does not provide a native SoundCloud player, stream or download. Do not claim every dua has a specific reward. |
