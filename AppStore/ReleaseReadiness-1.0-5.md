...
## Build 7 (18 September 2026)

Build 7 adds the tafsir-link fix, full-range Qur'an passage links, the read-only view grants and the dashboard CSP to build 6. 165 of 166 simulator tests pass (same pre-existing failure). Release archive `build/Haneen-1.0-7.xcarchive` reports 1.0 (7); `xcodebuild -exportArchive` with the app-store-connect upload options ended in "Upload succeeded" at 00:04 AEST (`build/Haneen-1.0-7-upload.log`). The same archive is installed on EZY for the requested recording. The review user exists in Supabase, confirmed and with a password. App Store Connect fields were not inspected or saved by this task (no signed-in browser); final notes are in `ReviewNotes-1.0-7.txt`. Not submitted for review.
