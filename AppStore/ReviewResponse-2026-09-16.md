...
- Morning/evening recording links: `Sakina/Audio/AdhkarRecording.swift`, shown by `PracticeDestination` in `Sakina/Duas/DuasView.swift`. Tapping **Listen on SoundCloud** opens the full original recording externally; there is no native SoundCloud player, stream extraction or audio download.
